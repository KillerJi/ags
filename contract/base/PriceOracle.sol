...
contract PriceOracle {
    ...
    function getUnderlyingPrice(address _aToken) external view returns (uint) {
        // USDT/USDC 1:1
        if(keccak256(abi.encodePacked((AToken(_aToken).symbol()))) == keccak256(abi.encodePacked(("USDT-AEGIS"))) || keccak256(abi.encodePacked((AToken(_aToken).symbol()))) == keccak256(abi.encodePacked(("USDC-AEGIS")))) {
            return 1e8;
        }
    ...
    function postUnderlyingPrice(address _aToken, uint _price) external {
        require(msg.sender == owner, "PriceOracle::postUnderlyingPrice owner failer");
