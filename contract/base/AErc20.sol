...
contract AErc20 is AToken, AErc20Interface {
    ...
    function initialize(address _underlying, AegisComptrollerInterface _comptroller, InterestRateModel _interestRateModel, uint _initialExchangeRateMantissa,
            string memory _name, string memory _symbol, uint8 _decimals, address payable _admin) public {
        admin = msg.sender;
        super.initialize(_name, _symbol, _decimals, _comptroller, _interestRateModel, _initialExchangeRateMantissa);
    ...
    function mint(uint _mintAmount) external returns (uint) {
        (uint err, uint item) = mintInternal(_mintAmount);
        return err;
    }
    ...
    function repayBorrow(uint _repayAmount) external returns (uint) {
        (uint err, uint actualRepayAmount) = repayBorrowInternal(_repayAmount);
        return err;
    }
    ...
    function repayBorrowBehalf(address _borrower, uint _repayAmount) external returns (uint) {
        (uint err, uint actualRepayAmount) = repayBorrowBehalfInternal(_borrower, _repayAmount);
        return err;
    }
     ...
     * @param _aTokenCollateral The market in which to seize collateral from the borrower
     * @return uint ERROR
     */
    function liquidateBorrow(address _borrower, uint _repayAmount, ATokenInterface _aTokenCollateral) external returns (uint) {
        (uint err, uint actualRepayAmount) = liquidateBorrowInternal(_borrower, _repayAmount, _aTokenCollateral);
        return err;
    }
    ...
    function doTransferIn(address _from, uint _amount) internal returns (uint) {
        ...
        require(success, "doTransferIn fail");

        uint balanceAfter = EIP20Interface(underlying).balanceOf(address(this));
        require(balanceAfter >= balanceBefore, "doTransferIn::balanceAfter >= balanceBefore fail");
    ...
    function doTransferOut(address payable _to, uint _amount) internal {
        ...
        require(success, "dotransferOut fail");
    }
}